...
| **P1** | 海关贸易数据底座 + GEO 定向营销联动 | 纵向/商业差异化 | 🟠 高 | 21.5d | P0 |
| **P2** | RAG 解析升级（Docling） | 横向 | 🟡 中 | 5d | P0 |
| **P2** | 分析层升级（Vanna） | 横向 | 🟡 中 | 5d | P0 |
| **P3** | 观测持久化（Langfuse） | 横向 | 🟢 低（需升配） | 6d | 升配 |

> 关键路径：P0(LiteLLM) → P1(情报+海关底座) → P2(RAG) → P3(观测)；P0(打包) 与 P1(培训) 可并行；P1-C 底座 MVP 可先于联动营销启动。
...
### P1-C：海关贸易数据底座（情报④强化）+ GEO 定向营销联动（营销⑤）

> **需求来源**：用户要求在情报模块整合全球海关数据（进口品类、采购商、港口），并在 GEO 模块基于该数据底座做定向推送/营销。
> **本节先给可行性结论，再给 WBS。**

#### 1. 可行性评估结论

**核心结论**：需求可行，但必须区分"两类海关数据"，否则会高估覆盖范围并低估合规成本。

- **Tier-1 官方统计海关数据（免费/聚合，无采购商名）**：覆盖全球 200+ 报告国（UN Comtrade 占全球商品贸易 99%+），含 HS 品类、贸易伙伴国、港口、金额、数量。**不含采购商/进口商企业名**（保密）。这是"全球有披露海关数据的国家都要"的真实可达范围。
- **Tier-2 提单（BOL）级数据（付费/部分免费，含采购商名）**：仅美国（CBP 依法公开）、印度（Zauba 免费），及拉美/越南/巴基斯坦等部分市场经商业平台（Panjiva/ImportGenius/Volza/ImportYeti）可得**出口商/进口商/通知方/起运港/目的港/HS/重量**。**绝非全球覆盖，且受再分发授权约束**。

**需求中的"采购商"字段只能来自 Tier-2，而 Tier-2 不覆盖"全球"。** 因此"全球披露国 + 采购商"二者不可兼得，产品中须明确分层。

**合规三道红线（须在设计期锁定）**：
1. **再分发授权**：Panjiva/ImportGenius 等 BOL 平台条款禁止原始记录再分发。产品应以"衍生情报/受众分群"（品类×港口×进口频次的评分画像）形态交付，而非原始提单查询，规避授权风险。
2. **隐私与反垃圾**：采购商为企业法人（通常不构成个人信息），但抓取联系人/邮箱即触 PIPL/GDPR/ePrivacy + CAN-SPAM/CASL。建议仅企业级触达，联系人经合规渠道补全并支持退订。
3. **制裁筛查**：受众分群上线前须过 OFAC/EU 制裁名单，复用现有 `geo_guard.py` 拦截。

#### 2. 数据层与覆盖矩阵

| 数据层 | 代表覆盖 | 含采购商名 | 成本 | 授权可商用性 | 接入方式 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Tier-1 官方统计** | UN Comtrade(200+国) + 各国门户(美/欧/巴西/墨/印/日/韩/台/澳/印尼/土/智/哥/秘…) | ❌ 仅品类/伙伴国/港口/金额 | 免费/极低成本(API key) | ✅ 政府公开数据 | API/CSV 定时抓取 |
| **Tier-2 提单 BOL** | 美国(CBP,最强)、印度(Zauba 免费)、拉美(秘/哥/智/哥斯达黎加)、越南、巴基斯坦 | ✅ Shipper/Consignee/Notify/港口/HS | 付费年费 3-8 万¥ 起；ImportYeti/Volza 部分免费 | ⚠️ 受再分发条款约束，需企业级授权 | 商业 API（再分发授权）或受约束抓取 |

**建议优先落地顺序**（按外贸 B2B 价值 + 可达性）：
1. **底座 MVP**：UN Comtrade 全球聚合（品类/伙伴国/港口，免费）+ 美国 BOL（ImportYeti 免费层，采购商级）+ 印度 BOL（Zauba 免费，采购商级）。覆盖"品类全球 + 采购商美/印"。
2. **扩展**：接入企业级 BOL API（Volza/ImportGenius 再分发授权）扩到拉美/越南/巴基斯坦；补 EU Comext、巴西 Comex Stat 等 Tier-1 门户。
3. **全球补齐**：视升配/分机接更多国家门户；采购商级仍以 Tier-2 覆盖国为上限。

#### 3. 架构与数据流（复用现有模块）

```
海关数据源(Tier-1 API / Tier-2 BOL API)
        │  customs_scraper (data_agent 新增 SourceType)
        ▼
归一化层：HS 字典 + 港口字典 + 采购商实体解析(buyer entity resolution)
        │  存储：SQLite(customs schema) + Qdrant(买家语义检索向量)
        ▼
合规层：GEO Guard(制裁筛查 + 企业级触达 + 退订)
        │
        ├─► 情报门户 CustomsView(品类/采购商/港口 检索+趋势)
        │
        └─► marketing_agent.customs_audience_connector
                  │ 分群(品类×港口×进口频次×增长)
                  ▼
             GEO 内容(geo_writer + multilingual + keyword_strategy)
                  │ 定向推送(push_service / 广告多渠道)
                  ▼
             ROI 归因(roi_predictor + performance_tracker)
```

**复用点**：`data_agent/scrapers/`（新增 `customs_scraper`）、`geo_guard.py`、`push_service.py`、`pipeline.py`；`marketing_agent/geo/*`、`content/*`、`ads/*`、`analytics/*`（新增 `customs_audience_connector`）。无新框架引入。

**资源约束提示**：海关数据体量远大于 RSS/网页。批量入库须作为**定时/离线 ETL**（scheduler 或独立 worker），避免拖垮现 2C/11G 单机；存储增长须监控，采购商向量入 Qdrant 复用现有实例。
...
| 任务 | 改动点 | 输出/验收 | 工作量 |
| :--- | :--- | :--- | :--- |
| C-1 数据源注册与合规清单 | `docs/` + `data_agent/customs_sources.yaml` | 分层数据源注册表 + 三道红线合规清单 | 0.5d |
| C-2 UN Comtrade 适配器 | `customs_scraper`(Tier-1) + `api_scraper` 复用 | 按 HS/伙伴国/港口拉取，归一化品类/金额 | 2d |
| C-3 BOL 商业适配器 | `customs_scraper`(Tier-2) + 字段归一化 | 美/印免费层接入；Shipper/Consignee/HS/港口落地 | 3d |
| C-4 实体归一化与存储 | `customs_models.py` + SQLite schema + Qdrant | 采购商实体解析、HS/港口字典、语义检索 | 2d |
| C-5 GEO Guard 合规接入 | `geo_guard.py` 扩展 | 制裁名单拦截 + 企业级触达开关 + 退订 | 1d |
| C-6 情报门户 CustomsView | `intelligence-portal` | 品类/采购商/港口检索 + 趋势视图 | 2d |
| C-7 测试 + 文档 | `data_agent/tests/` | 适配器集成测试 + 合规用例 | 1.5d |

**C2 子阶段 — GEO 定向营销联动（⑤）**

| 任务 | 改动点 | 输出/验收 | 工作量 |
| :--- | :--- | :--- | :--- |
| C-8 受众连接器 | `marketing_agent/customs_audience_connector.py` | 由海关底座分群(品类×港口×频次×增长) | 2d |
| C-9 定向内容生成 | 复用 `geo_writer`+`multilingual`+`keyword_strategy` | 按分群生成品类/买家画像定向文案 | 2d |
| C-10 定向推送链路 | 复用 `push_service`/营销 push | 邮件/门户/广告多渠道定向触达 | 2d |
| C-11 ROI 归因 | 复用 `roi_predictor`+`performance_tracker` | 转化归因到海关受众分群 | 1.5d |
| C-12 测试 + 文档 | `marketing_agent/tests/` | 联动集成测试 + 配置手册 | 1.5d |

**工作量合计 ≈ 21.5d**；建议 C1 先行（MVP=Comtrade+美/印），C2 在 C1 验收后启动；扩国在升配/分机后。

**风险**：海关 BOL 再分发授权（R7）；采购商级营销隐私/反垃圾（R8）。**缓解**：以衍生情报形态交付、企业级触达、制裁筛查、离线 ETL 保护主机。
...
| R7 海关 BOL 再分发授权 | 高 | 中 | 以衍生情报/受众分群形态交付，不重分发原始提单；仅采购企业级授权 API |
| R8 采购商级营销隐私/反垃圾 | 中 | 中 | 仅企业级触达；联系人合规补全+退订；上线前过制裁名单(GEO Guard) |
...
| **M2 情报可信+培训+海关底座** | P1-A + P1-B + P1-C 完成 | 50.5d |
| **M3 RAG+分析质量抬升** | P2-A + P2-B 完成 | 60.5d |
| **M4 观测持久化（升配后）** | P3 完成 | 66.5d |

> 单人节奏约 13-14 周；P0 两项并行可压缩。P1-C 体量最大，建议底座 MVP(Comtrade+美/印)先行、扩国与联动营销分期。P3 受升配节奏制约，可独立排期。
...
- P1-C 海关数据底座直接强化 ④情报 内核，并经 GEO 联动放大 ⑤营销 差异化，是纵向护城河的数据燃料。
...
*修订记录：v1.1 2026-07-13 新增 P1-C 海关贸易数据底座 + GEO 定向营销联动（可行性评估 + Tier1/Tier2 分层 + 覆盖矩阵 + WBS C1/C2）；同步更新 WBS 总览、风险 R7-R8、里程碑 M2-M4*
